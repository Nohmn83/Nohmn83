# 👋 Hello, I'm Noh Hyeonmin.

## 🎓 About Me
- 🏫 **한신대학교(Hanshin University)** AI·SW계열 재학 중
- 💻 **AI 알고리즘**과 **소프트웨어 개발**에 관심이 많습니다.
- 💡 기술을 통해 일상의 불편함을 해결하는 것을 좋아합니다.

---
## 👋 안녕하세요! 예비 개발자 '노현민' 입니다.

한신대학교 AI·SW학부에서 소프트웨어의 기초를 다지며, '어제보다 더 나은 코드' 를 작성하기 위해 꾸준히 성장하고 있습니다.


---

## 🛠 Tech Stack
현재는 기초를 탄탄히 쌓는 단계입니다.

- **Language:** `C` (학습 중)
- **Tools:** `Git`, `Notion`
- **Interest:** `AI`, `Algorithm`, `Web Development`

---

## 📚 Academic Journey
현재 대학 과정에서 배우는 내용들을 바탕으로 문제 해결 능력을 기르고 있습니다.

- **컴퓨팅 사고와 SW 코딩:** 논리적 알고리즘 설계 연습
- **C언어:** 프로그래밍의 근간이 되는 메모리 구조 학습
- **AI·SW 개론:** 최신 기술 트렌드 및 산업 이해

---

## 📈 Growth Roadmap
꾸준히 기록하며 성장하는 개발자가 되겠습니다.

| 학년 | 핵심 키워드 | 세부 목표 |
| :--- | :--- | :--- |
| **1학년** | 기초 체력 | C언어 마스터, 알고리즘 기초 |
| **2학년** | 영역 확장 | Python/Java, 웹/앱 프로젝트 |
| **3학년** | 심화 학습 | 데이터베이스, 네트워크, AI/ML |
| **4학년** | 완성 | 현장 실습 및 취업 포트폴리오 |

---

> "배우는 것을 멈추지 않는 개발자가 되겠습니다."

## 🛠️ Tech Stacks
### 🚀 Languages & Frameworks
<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=Python&logoColor=white"/>
  <img src="https://img.shields.io/badge/C-A8B9CC?style=flat-square&logo=C&logoColor=white"/>
  <img src="https://img.shields.io/badge/Java-007396?style=flat-square&logo=OpenJDK&logoColor=white"/>
  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=PyTorch&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05033?style=flat-square&logo=Git&logoColor=white"/>
</p>

### 🔧 Tools & Platforms
<p>
  <img src="https://img.shields.io/badge/VS%20Code-007ACC?style=flat-square&logo=Visual%20Studio%20Code&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=GitHub&logoColor=white"/>
  <img src="https://img.shields.io/badge/Google%20Colab-F9AB00?style=flat-square&logo=Google%20Colab&logoColor=white"/>
</p>

---

## 📊 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=[본인ID]&show_icons=true&theme=radical" alt="GitHub Stats" />
</p>
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=[본인ID]&layout=compact&theme=radical" alt="Top Langs" />
</p>

---

## 📫 Contact
- **Email** : nogusals@hs.ac.kr
- **Blog** : 

<div align="right">
  <img src="https://komarev.com/ghpvc/?username=[본인ID]&label=Profile%20views&color=0e75b6&style=flat" alt="Views" />
</div>
